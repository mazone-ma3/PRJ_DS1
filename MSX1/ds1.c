...
enum {
	TILE_WALL = 0x00,   // 壁
	TILE_FLOOR = 0x04,   // 床
	TILE_PLAYER = 0x08,   // プレイヤー
	TILE_NORMAL = 0x0c,   // 通常パネル（S） - シンプルブロック
...
void copy_to_vram3(char *src, short dst, int size)
{
	while(size--){
		vpoke(dst, *(src));
		vpoke(1024*2+dst, *(src));
		vpoke(1024*4+dst, *(src));
		src++;
		dst++;
	}
}
...
void put_chr16(int x, int y, char chr) {
	switch(chr){
		...
		case TILE_SLIME:
			vdp_put_sprite_16(0,x*16, y*16, 12,7);
			vdp_put_sprite_16(1,x*16, y*16, 16,5);
			vdp_put_sprite_16(2,x*16, y*16, 20,0);
			break;
		...
		default:
			put_chr8(x * 2, y * 2, chr);
			put_chr8(x * 2 , y * 2 + 1,  chr+1);
			put_chr8(x * 2 + 1, y * 2,  chr+2);
			put_chr8(x * 2 + 1, y * 2 + 1, chr+3);
			break;
	}
}
...
*/
char tile_wall[8] = {0x10,0x10,0x10,0xff,0x01,0x01,0x01,0xff,};

char tile_player[3][32]  = {
{0x07,0x06,0x0c,0x0c,0x08,0x00,0x00,0x00,0x1f,0x3f,0x3f,0x2f,0x0f,0x0f,0x3f,0x7e,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x80,0x80,0x80,0x80,0x80,
},
{0x00,0x00,0x02,0x02,0x06,0x05,0x05,0x03,0x00,0x00,0xc0,0xc0,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xe0,0xa0,0xa0,0xc0,0x00,0x00,0x03,0x03,0x00,0x00,0x00,0x00,
},
{0x00,0x03,0x01,0x01,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xe0,0xe0,0xf0,0xf0,0x10,0x00,0x00,0x00,0xf0,0xfc,0xfc,0x70,0x70,0x70,0x7c,0x7e,
},
};


char tile_normal[32] = {0x7f,0xff,0xfe,0xfc,0xf8,0xfe,0xee,0xc0,0xc0,0xee,0xfe,0xf8,0xfc,0xfe,0xff,0x7f,0xfe,0xff,0x7f,0x3f,0x1f,0x7f,0x77,0x03,0x03,0x77,0x7f,0x1f,0x3f,0x7f,0xff,0xfe,};

char tile_gravity[32] = {0x7f,0xff,0xfe,0xfe,0xfe,0xfe,0xfe,0xfe,0xfe,0xfe,0xce,0xe6,0xf0,0xfc,0xff,0x7f,0xfe,0xff,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x73,0x67,0x0f,0x3f,0xff,0xfe,};

char tile_goal[32] = {0xff,0xff,0xff,0xf0,0xe7,0xcf,0xcf,0xcf,0xcf,0xcf,0xcf,0xe7,0xf0,0xff,0xff,0xff,0xff,0xff,0xff,0x0f,0xe7,0xe7,0xff,0xff,0x03,0xf3,0xf3,0xc3,0x13,0xff,0xff,0xff,};

char tile_hollow[32] = {0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,};

char tile_slime[3][32]  = {
{0x00,0x00,0x00,0x00,0x00,0x00,0x07,0x0f,0x3f,0x3f,0xfb,0xfb,0xff,0xff,0xff,0x1e,0x00,0x00,0x00,0x00,0x00,0x00,0xe0,0xf0,0xfc,0xfc,0xdf,0xde,0xfd,0xea,0x55,0xa8,
},
{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x02,0x15,0xaa,0x50,
},
...
*/
unsigned char colordata[8][32] = {
//	0x5F,0x2F,0x8a,0x7f,0x8f,0x50,0x70
//	0x5F,0x00,0x2F,0x8a,0x7f,0x8f,0x50,0x70
	...
	{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,},
	{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,},
	{0x20,0x20,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x20,0x20,0x20,0x20,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x20,0x20,},
	{0x70,0x70,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x70,0x70,0x70,0x70,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x7f,0x70,0x70,},
...
void define_tiles(void) {
	int i,j;
	copy_to_vram3(tile_wall, (TILE_WALL * 8), 8); 
	copy_to_vram3(tile_floor, (TILE_FLOOR * 8), 32);
/*	copy_to_vram(tile_floor, ((TILE_FLOOR+1) * 8), 8);
	copy_to_vram(tile_floor, ((TILE_FLOOR+2) * 8), 8);
	copy_to_vram(tile_floor, ((TILE_FLOOR+3) * 8), 8);*/
//	copy_to_vram(tile_player[0], (TILE_PLAYER * 8), 32);
	copy_to_vram3(tile_normal, (TILE_NORMAL * 8), 32);	//通常パネル
	copy_to_vram3(tile_gravity,(TILE_GRAVITY * 8), 32);	// 重力パネル
	copy_to_vram3(tile_goal,	(TILE_GOAL * 8), 32);
//	copy_to_vram(tile_slime[0], (TILE_SLIME * 8), 32);
	copy_to_vram3(tile_hollow, (TILE_HOLLOW * 8), 32);	//	:新ブロック
...
//		vpoke(8213 + i, colordata[i]);
		for(j = 0; j < 8*4; ++j){
			vpoke(0x2000 + i*8*4+j, colordata[i][j]);
			vpoke(0x2000 + 2*1024 + i*8*4+j, colordata[i][j]);
			vpoke(0x2000 + 4*1024 +i*8*4+j, colordata[i][j]);
		}
	}

	vdp_set_sprite_mode(sprite_large);

	vdp_set_sprite_16(0, tile_player[0]);
	vdp_set_sprite_16(4, tile_player[1]);
	vdp_set_sprite_16(8, tile_player[2]);

	vdp_set_sprite_16(12, tile_slime[0]);
	vdp_set_sprite_16(16, tile_slime[1]);
	vdp_set_sprite_16(20, tile_slime[2]);
