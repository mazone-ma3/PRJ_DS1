...
void fill_vram(char *adr, int size, char pattern)
{
	while(size--)
		vpoke(adr++, pattern);
}

void msx_beep(int a,int b, int c)
{
__asm
	push	af
	push	hl
...
// VRAM直書き
void msx_print_at(int x, int y, char *str) {
    char chr;
...
void main() {
    ...
    msx_set_mode(1);
    define_tiles();
    fill_vram(0x1800, 0x01, 32*24);  // 初回クリア（床で埋める）
    ...
    while (1) {
        if (game_mode == 0) {
            ...
            if ((get_stick(0) == 3) && try_move(1, 0)) { msx_wait(4); update_objects(); play_sound_effect(); }
            if ((get_stick(0) == 7) && try_move(-1, 0)) { msx_wait(4); update_objects(); play_sound_effect(); }
            if ((get_stick(0) == 5) && try_move(0, 1)) { msx_wait(4); update_objects(); play_sound_effect(); }
            if ((get_stick(0) == 1) && try_move(0, -1)) { msx_wait(4); update_objects(); play_sound_effect(); }
...
void gravity_fall() {
    ...
    if (gravity_y + 1 < MAP_H && can_move(gravity_x, gravity_y + 1)) {
        ...
        if (gravity_x == goal_x && gravity_y == goal_y) {
            update_objects();
            msx_print_at(10, 10, "STAGE CLEAR!!");
            msx_wait(60);
            msx_cls();
            current_stage++;
...
void draw_background() {
    ...
    for (y = 0; y < MAP_H; y++) {
        for (x = 0; x < MAP_W; x++) {
            ...
            if (c == '#') vpoke(0x1800 + x * 2 + y * 32, 0);
            else if (c == 'G') vpoke(0x1800 + x * 2 + y * 32, 5);
            else vpoke(0x1800 + x * 2 + y * 32, 1);
        }
    }
}
...
void update_objects() {
    ...
    // 前の位置を床に戻す
    if (old_player_x >= 0) vpoke(0x1800 + old_player_x * 2 + old_player_y * 32, 1);
    if (old_gravity_x >= 0) vpoke(0x1800 + old_gravity_x * 2 + old_gravity_y * 32, 1);
    for (i = 0; i < panel_count; i++) {
        if (old_panels_x[i] >= 0) vpoke(0x1800 + old_panels_x[i] * 2 + old_panels_y[i] * 32, 1);
    }

    // 新しい位置に描画
    if (player_x >= 0)
        vpoke(0x1800 + player_x * 2 + player_y * 32, 2);  // プレイヤー

    if (gravity_x >= 0) vpoke(0x1800 + gravity_x * 2 + gravity_y * 32, 4);  // 重力パネル（B）

    for (i = 0; i < panel_count; i++) {
        vpoke(0x1800 + panels_x[i] * 2 + panels_y[i] * 32, 3);  // 通常パネル（S）
    }
...
void start_battle() {
    ...
    msx_cls();
    // スライム表示（中央上部に）
    vpoke(0x1800 + 16 * 1 + 4 * 32, 6);  // スライムキャラコード6
    msx_print_at(5, 8, battle_msg);
    msx_print_at(5, 12, "Press SPACE to attack");
}
    ...
    if (get_trigger(0)) {
        ...
        if (enemy_hp <= 0) {
            ...
            if (exp >= level * 20) {
                ...
                player_hp += 5;
                player_atk += 2;
                msx_print_at(5, 14, "Level Up!");
                msx_wait(30);
            }
            msx_cls();
            strcpy(battle_msg, "Enemy defeated!");
            msx_print_at(5, 10, battle_msg);
            msx_wait(60);
            msx_cls();
            game_mode = 0;
            draw_background();
            ...
            msx_cls();
            // スライム再描画（バトル継続時）
            vpoke(0x1800 + 16 * 1 + 4 * 32, 6);
            msx_print_at(5, 8, "Enemy defeated? No!");
            msx_print_at(5, 10, battle_msg);
            msx_print_at(5, 12, "Press SPACE to attack");
        }
        play_sound_effect();
...
void play_sound_effect() {
    msx_beep(0x80, 1000, 10);  // シンプルなビープ音
}