/* Bitmap(8 colors) to X1 & FM77AV .grp Converter By m@3 */
/* (デジタル8色) */
...
unsigned char write_pattern[X1_WIDTH / 2 * X1_HEIGHT + 2];
unsigned char pattern[10+1];

unsigned char grp_buffer[X1_WIDTH][X1_HEIGHT];
unsigned char X1_buffer[X1_WIDTH / 8 / 2][X1_HEIGHT][3];
...
int conv(int arg, char *bitmapfil, char *grpfil, char *maskfil)
{
	...
	if((width > X1_WIDTH * 2) || (height > (X1_HEIGHT * 2))){
		fclose(stream[0]);
...
//	datasize = read_pattern[0x0] + 256 * (read_pattern[0x1] + 256 * (read_pattern[0x2]  +  256 * read_pattern[0x3]));
	...
	fread(read_pattern, 1, width * height, stream[0]);	/* ピクセルデータ */
...
//			write_pattern[i + (height - j - 1) * (X1_WIDTH / 2)] = read_pattern[k++];
				if(arg < 4){
					grp_buffer[i * 1 + 0][height / 2 - 1 - (j / 2)] = cvx1_tbl[read_pattern[k] % 16];
//					grp_buffer[i * 2 + 1][height / 2 - 1 - (j / 2)] = cvx1_tbl[read_pattern[k] / 16];
				}else{
					grp_buffer[i * 1 + 0][height / 2 - 1 - (j / 2)] = conv_tbl[read_pattern[k] % 16];
...
///////////////////////////////////////////////////////////////////////////////
	...
	write_size = X1_WIDTH / 8 / 2 * X1_HEIGHT * 3;
...
{
	if (argc < 3){
		printf("Bitmap 8colors to X1 and FM .grp file Converter. (low-rezo)\n");
		printf("Usage : %s Inputfile(Bitmap) Outputfile(X1 / FM Graphic Data) [Outputfile(mask Data)]\n", argv[0]);
		printf("Counting from 0, odd-numbered pixels and lines are omitted.\n");
